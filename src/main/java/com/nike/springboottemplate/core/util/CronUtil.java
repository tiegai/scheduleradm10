...
import static com.nike.springboottemplate.core.util.UtcLocalDateUtil.*;
...
public class CronUtil {
    ...
    public static String cronListNextStart(List<String> cronStrList,Date startDate){
        ...
        for(int i=0;i<cronStrList.size();i++){
            nextStartList.add(strToDate(cronNextStart(cronStrList.get(i),startDate)));
        }
        Collections.sort(nextStartList);
        return localStrToUtcStr(dateToStr(nextStartList.get(0)));
    }

    public static void main(String[] args){
        System.out.println(cronNextStart("0 50 18 * * ?",new Date()));
        System.out.println(localStrToUtcStr(cronNextStart("0 50 18 * * ?",new Date())));
        ...
        System.out.println(cronListNextStart(cronStrList,strToDate("2022-11-25 16:55:00")));
    }
}
